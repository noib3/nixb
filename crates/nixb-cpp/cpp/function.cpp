...
using RustCallback = void (*)(void*, nix_c_context*, EvalState*, nix_value**, nix_value*);
...
#else
using RustCallback = void (*)(void*, nix_c_context*, nix::EvalState*, nix::Value**, nix::Value*);
#endif
...
struct ExprRustLambda : nix::ExprLambda {
    ...
    void eval(nix::EvalState& state, nix::Env& env, nix::Value& v) override {
        ...
        #ifdef NIX_BINDINGS_TARGET_NIX_2_34
        if (!v_tmp.isValid()) {
            throw nix::Error("Implementation error in function callback: return value was not initialized");
        }

        if (v_tmp.type() == nix::nThunk) {
            throw nix::Error("Implementation error in function callback: return value must not be a thunk");
        }
...
/// - `callback`: Called each time the function is invoked (after full application)
/// - `on_drop`: Called when the function is GC'd (can be nullptr for static userdata)
///
/// # Callback Contract
///
...
nix_err init_function(
    ...
    try {
        ...
        if (arity == 0 || arity > nix::maxPrimOpArity)
            throw nix::Error("function arity must be between 1 and %d", (int) nix::maxPrimOpArity);
        if (!args)
