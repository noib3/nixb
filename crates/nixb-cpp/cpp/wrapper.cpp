...
#ifdef NIX_BINDINGS_TARGET_NIX_2_34
...
#ifndef NIX_BINDINGS_TARGET_NIX_2_34
...
#ifdef NIX_BINDINGS_TARGET_NIX_2_34
nix_value*
get_attr_byname_lazy_no_incref(const nix_value* value, EvalState* state, const char* name) {
...
#else
nix::Value*
get_attr_byname_lazy_no_incref(const nix::Value* value, nix::EvalState* state, const char* name) {
...
#ifdef NIX_BINDINGS_TARGET_NIX_2_34
nix_value*
get_list_byidx_lazy_no_incref(const nix_value* value, unsigned int ix) {
    return borrow_nix_value(check_value_not_null(value).listView()[ix], *value->mem);
}
#else
nix::Value*
get_list_byidx_lazy_no_incref(const nix::Value* value, unsigned int ix) {
...
#ifdef NIX_BINDINGS_TARGET_NIX_2_32
...
extern "C" nix_err init_thunk(
...
#else
    void (*on_force_once)(nix_c_context*, nix::EvalState*, nix::Value*, void*),
#endif
    void (*on_drop)(void*)
) {
    // Custom Expr subclass that invokes a Rust callback when evaluated.
    // Note: This is defined outside the try block since struct definitions cannot throw.
    struct ExprRustCallback : nix::Expr {
        ...
        ExprRustCallback(
            void* data,
#ifdef NIX_BINDINGS_TARGET_NIX_2_34
            void (*callback)(nix_c_context*, EvalState*, nix_value*, void*),
#else
            void (*callback)(nix_c_context*, nix::EvalState*, nix::Value*, void*),
#endif
            void (*drop)(void*)
        ...
        void eval(nix::EvalState & state, nix::Env &, nix::Value & v) override {
            // RAII guard to reset is_evaluating on scope exit (even during exceptions)
        ...
        void bindVars(
            nix::EvalState &,
            const std::shared_ptr<const nix::StaticEnv> &
        ) override {}
    };
